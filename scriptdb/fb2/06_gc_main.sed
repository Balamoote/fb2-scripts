...
s=(\w|\xcc\x81)[ \xc2\xa0]?-[ \xc2\xa0]?(то|либо|нибудь|ка|таки|де|за)\b=\1-\2=gI
s=\b(во?|кое)[ \xc2\xa0]?-[ \xc2\xa0]?(\w)=\1-\2=gI
...
#Меняем короткие тире на дефисы в 
#s=([[:alpha:]])–(то|таки|де|либо|нибудь|за)\b=\1-\2=gI
#s=(\b(по|псевдо|кое|во?)|(но|не))–([[:alpha:]])=\1-\4=gI
s=([^0-9VXICLDM ])–([^0-9VXICLDM ])=\1-\2=g
...
# замена дефисов на тире в начале строки
s=(<[pv]>)[ \xc2\xa0]?($aib)?[-–]{1,2}[ \xc2\xa0]?($aib)?[ \xc2\xa0]?([^0-9<])=\1\2— \3\4=gI
...
# отбивка на полукегельную от цифр (неразрывный пробел) знаков параграфа, номера
...
# градус Цельсия
s=\b([0-9]+)[ \xc2\xa0]?[oо°]\s?[CС]([^[:alpha:]])?=\1\xc2\xa0°C\2=g
